...
```bash
pi install git:github.com/cfuentessalgado/pi-mcp-adapter
```
...
Run `/mcp setup` after installation to scan for host-specific configs and add missing compatibility imports to the Pi agent dir (`~/.pi/agent/mcp.json` by default, or `$PI_CODING_AGENT_DIR/mcp.json` when set).
