...
function printHelp(log = console.log) {
  ...
  log("  pi install git:github.com/cfuentessalgado/pi-mcp-adapter\n");
...
export async function main(argv = process.argv.slice(2), log = console.log, error = console.error) {
  ...
  if (command === "install") {
    ...
    error("Use `pi install git:github.com/cfuentessalgado/pi-mcp-adapter` instead, then optionally run `pi-mcp-adapter init`.");
    return 1;
  }
